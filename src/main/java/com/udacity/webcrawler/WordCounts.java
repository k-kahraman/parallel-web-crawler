...
final class WordCounts {
  ...
  private static final class WordCountComparator implements Comparator<Map.Entry<String, Integer>> {
    @Override
    public int compare(Map.Entry<String, Integer> stringIntegerEntry, Map.Entry<String, Integer> b) {
      if (!stringIntegerEntry.getValue().equals(b.getValue())) {
        return b.getValue() - stringIntegerEntry.getValue();
      }
      if (stringIntegerEntry.getKey().length() != b.getKey().length()) {
        return b.getKey().length() - stringIntegerEntry.getKey().length();
      }
      return stringIntegerEntry.getKey().compareTo(b.getKey());
    }
  }
